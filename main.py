...
class EmployeeManagementApp:
    def __init__(self, master):
        ...
        self.tree["columns"] =("ID", "FullName", "Phone", "Email", "Salary")
        self.tree.heading("ID", text="ID")
        self.tree.heading("FullName", text="FullName")
    ...
    def create_widgets(self):
        ...
        # To store the last action for undo
        self.last_action = None

    def add_employee(self):
        name = simpledialog.askstring("Input", "Enter employee fullname:")
        ...
        cursor = self.conn.cursor()
        cursor.execute("INSERT INTO employees (fullname, phone, email, salary) VALUES (?, ?, ?, ?)", (name, phone, email, salary))
        self.conn.commit()
        self.update_treeview()
    ...
    def update_employee(self):
        ...
        if employee:
            name = simpledialog.askstring("Input", "Enter updated employee fullname:", initialvalue=employee[1])
            ...
            cursor.execute("UPDATE employees SET fullname=?, phone=?, email=?, salary=? WHERE id=?", (name, phone, email, salary, emp_id))
            self.conn.commit()
            self.update_treeview()
    ...
    def search_employee(self):
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM employees WHERE fullname=?", (name,))
